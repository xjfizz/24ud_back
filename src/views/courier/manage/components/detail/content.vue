  ...
  <section class="detail-wrapper">
    <dl class="detail-item">
      ...
      <dd>
        ...
        <span class="detail-text">{{row.name}} {{row.sex ? '男' :'女'}} {{row.phone}}</span>
      </dd>
      <dd>
      ...
      <dd>
        ...
        <span class="detail-text">{{row.emergencyPhone}}</span>
      </dd>
      <template v-if="row.type === 0">
        <dd>
          <span class="detail-label">所属站点</span>
          <span class="detail-text">{{row.oid | formatId2Label(id2labels) }}</span>
        </dd>
        <dd>
          <span class="detail-label">所属区域</span>
          <span class="detail-text">{{row.areaId | formatId2Label(id2labels) }}</span>
        </dd>
      </template>
      <template v-else>
<!--        <dd>-->
